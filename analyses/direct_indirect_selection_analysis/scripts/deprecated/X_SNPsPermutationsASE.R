...
run_snp_permutation_tests <- function( dataset, variable, snp_list, nb_reps )
{
  ...
  hist(permutations, xlim=range(c(permutations,original_score)))
  abline(v=original_score, col="red")
  return(1-qval)
}
...
############
parallel_genes = unique(filter(SNP_dataset, Parallelism>=5)$gene)
parallel_snps  = unique(filter(SNP_dataset, gene%in%parallel_genes)$ID)
############
...
pval1 = run_snp_permutation_tests(SNP_dataset, "is_ASE", significant_sg_pool_cis, 10000)
pval2 = run_snp_permutation_tests(SNP_dataset, "is_ASE", highest_sg_pool_cis, 10000)
pval3 = run_snp_permutation_tests(SNP_dataset, "is_ASE", evol_response_pool_cis, 10000)
pval4 = run_snp_permutation_tests(SNP_dataset, "is_ASE", common_pool_cis, 10000)
pval5 = run_snp_permutation_tests(SNP_dataset, "is_ASE", hub_genes_cis, 10000)
pval6 = run_snp_permutation_tests(SNP_dataset, "is_ASE", parallel_snps, 10000)

#------------------------------#
# 4) Save the results          #
#------------------------------#
D = data.frame(
  c("significant_sg_pool", "highest_sg_pool", "evol_response_pool", "common_pool", "hub_genes", "parallel_snps"),
  c(pval1, pval2, pval3, pval4, pval5, pval6)
)
names(D) = c("snp_pool", "pvalue")
write.table(D, "./analyses/direct_indirect_selection_analysis/data/SNP_ASE_permutations.txt", row.names=F, col.names=T, quote=F)
